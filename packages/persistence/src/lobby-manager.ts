...
import { GameState, Player, GamePhase, Table } from '@primo-poker/shared'
import type { TableFilters, TableListing, PublicPlayerInfo, LobbyTableConfig, LobbyJoinResult, ReservationResult, TableStats } from '@primo-poker/shared'
...
export class LobbyManager {
  ...
  async getAvailableTables(filters: TableFilters = {}): Promise<TableListing[]> {
    ...
    if (filters.searchTerm) {
      ...
      tables = tables.filter(t => 
        ...
        t.playerList.some((p: PublicPlayerInfo) => p.username.toLowerCase().includes(term))
      )
    }
   ...
   * Join a table
   */
  async joinTable(tableId: string, playerId: string, password?: string): Promise<LobbyJoinResult> {
  ...
  async leaveTable(playerId: string): Promise<boolean> {
    ...
    table.playerList = table.playerList.filter((p: PublicPlayerInfo) => p.playerId !== playerId)
  ...
  async updateTableStats(tableId: string, table: Table): Promise<void> {
    ...
    tableListing.currentPlayers = tableListing.playerList.filter((p: PublicPlayerInfo) => p.isActive).length
  ...
  async removeTable(tableId: string): Promise<boolean> {
    ...
    table.playerList.forEach((player: PublicPlayerInfo) => {
  ...
  private findAvailableSeat(table: TableListing): number {
    const occupiedSeats = new Set(table.playerList.map((_: PublicPlayerInfo, index: number) => index))
