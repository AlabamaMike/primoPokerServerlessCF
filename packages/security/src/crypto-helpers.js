...
export class CryptoHelpers {
    static ENTROPY_POOL_SIZE = 256;
    static REFRESH_INTERVAL = 300000; // 5 minutes
    static MAX_COUNTER = 0xFFFFFFFF;
    /**
     ...
     */
    static generateSecureInteger(min, max) {
        if (min >= max) {
     ...
     */
    static async deriveKey(inputKeyMaterial, salt, info, outputLength) {
        const algorithm = { name: 'HKDF', hash: 'SHA-256' };
        // Import the input key material
        const key = await crypto.subtle.importKey('raw', inputKeyMaterial, algorithm, false, ['deriveKey', 'deriveBits']);
        // Derive the key
     ...
     */
    static mixEntropy(sources) {
        if (sources.length === 0) {
            throw new Error('At least one entropy source required');
        }
        // Concatenate all entropy sources with metadata
        const combined = new Uint8Array(sources.reduce((total, source) => total + source.value.length + 16, 0));
        let offset = 0;
     ...
     */
    static async collectEntropy(tableId, playerId) {
        const sources = [];
     ...
     */
    static async createRandomState(tableId) {
     ...
     */
    static async refreshRandomState(state, tableId, force = false) {
        const now = Date.now();
     ...
     */
    static async generateFromState(state, length) {
