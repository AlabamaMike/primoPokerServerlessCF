...
export class SecureShuffle {
    static ALGORITHM_VERSION = 'secure-fisher-yates-v1';
    static MAX_ARRAY_SIZE = 10000;
    /**
     * Performs cryptographically secure Fisher-Yates shuffle
     * Eliminates modulo bias through rejection sampling
     */
    static async shuffle(array, recordSwaps = false) {
        if (array.length === 0) {
     ...
     */
    static async generateUnbiasedIndex(max) {
        if (max <= 1) {
            return {
                value: 0,
                entropyUsed: 0,
     ...
     */
    static async verifyShuffleProof(originalArray, shuffledArray, proof) {
        const errors = [];
        // Basic validation
        ...
        if (proof.swapSequence && errors.length === 0) {
            try {
                const verified = await this.verifySwapSequence(originalArray, shuffledArray, proof.swapSequence);
                if (!verified) {
     ...
     */
    static async verifySwapSequence(originalArray, shuffledArray, swapSequence) {
        ...
        for (let i = 0; i < swapSequence.length; i++) {
            ...
            if (arrayIndex > 0) {
                const expectedIndex = await this.verifyRandomIndex(arrayIndex + 1, swap.indexB, swap.entropyBytes);
                if (!expectedIndex) {
                    return false;
                }
            }
     ...
     */
    static async verifyRandomIndex(max, expectedIndex, entropyBytes) {
        if (max <= 1) {
     ...
     */
    static analyzeShuffleQuality(shuffleResults, originalArray) {
        const warnings = [];
...
//# sourceMappingURL=secure-shuffle.js.map