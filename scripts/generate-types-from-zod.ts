...
function main() {
  ...
  console.log(`📊 Exported schemas: ${exportedSchemas.length}`);
  
  if (exportedSchemas.length === 0) {
    console.log('❌ No exported Zod schemas found!');
    return;
  }
  
  console.log('\n✏️  Generating type registry...');
  const typesContent = generateTypesFile(allSchemas);
  const typesPath = path.join(OUTPUT_DIR, 'types.ts');
  fs.writeFileSync(typesPath, typesContent);
  console.log(`✅ Type registry written to: generated/types.ts`);
  
  console.log('\n✏️  Generating validation utilities...');
  ...
  console.log(`✅ Validation utilities written to: generated/validation.ts`);
...
`;
  ...
  console.log(`✅ Index file written to: generated/index.ts`);
  ...
  if (!sharedIndexContent.includes('./generated')) {
    ...
    console.log('✅ Updated shared/src/index.ts with generated exports');
  }
