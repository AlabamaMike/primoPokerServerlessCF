...
// Import AuditLog type from persistence package
import type { AuditLog } from '@primo-poker/persistence';
...
export class RNGAuditStorage {
  ...
  async generateAnalytics(tableId: string, days: number = 7): Promise<{
    ...
    for (const log of logs) {
      ...
      // Entropy total
      totalEntropyUsed += log.entropyUsed;
      
      // Daily stats
      const dateParts = new Date(log.timestamp).toISOString().split('T');
      const date = dateParts[0] || new Date(log.timestamp).toISOString().substring(0, 10);
      let dayStats = dailyStats.get(date);
      if (!dayStats) {
        dayStats = { operations: 0, entropyUsed: 0 };
        dailyStats.set(date, dayStats);
      }
      dayStats.operations++;
      dayStats.entropyUsed += log.entropyUsed;
    }
  ...
  private async updateIndex(
    ...
    let index: AuditIndex = { tableId, date: date || '', batches: [] };
    try {
  ...
  private detectSuspiciousPatterns(logs: AuditLog[]): string[] {
    ...
    for (let i = 1; i < logs.length; i++) {
      ...
      if (current && previous && current.operation === previous.operation) {
        const key = current.operation;
